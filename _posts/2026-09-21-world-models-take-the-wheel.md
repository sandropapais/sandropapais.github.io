...
The term *world model* gets slapped on three different stacks, and the conversation gets muddled when they're treated as the same thing. One clean way to tell them apart is where the action sits in the input-output loop. The same split applies whether the agent is a robot arm or a car.
...
Both routes win on data efficiency and generalization more than on peak leaderboard scores, using supervision that is close to free: SfM, synthetic renders, LiDAR logs, or video. The open fight is representation: generative VAE latents versus JEPA-style abstract states versus explicit geometry ([PointDiT](https://arxiv.org/abs/2607.02515) on point maps); [RAE-NWM](https://arxiv.org/abs/2603.09241) shows the navigation analogue by predicting rollouts in dense DINOv2 tokens (~79% on Habitat image-goal). For collision avoidance, explicit geometry still has the stronger case.
